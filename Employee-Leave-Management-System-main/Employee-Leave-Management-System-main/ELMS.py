...
conn.execute(
    "CREATE TABLE IF NOT EXISTS balance (employee_id text,sickleave int,maternityleave int,emergencyleave int)")
conn.execute(
    "CREATE TABLE IF NOT EXISTS STATUS (leave_id int,employee_id text,leave text, cur_date text, Date1 text,Date2 text,days int,status text, hrstatus text)")
conn.execute(
    "CREATE TABLE IF NOT EXISTS employee (register_id text, employee_id text, Name text, ContactNumber text, employee_mail text, employee_blood_group text, employee_address text, Password text)")
...
class AdminClass:
    ...
    def adminwindow(self):
        ...
        informationEmployee = Button(adminmainwindow, text='All Employee information',
                                     command=self.EmployeeAllInformationWindow, bd=12, relief=GROOVE, fg="black",
                                     bg="lightblue", font=("Castellar", 36, "bold"), activebackground="lightblue")
        informationEmployee.pack(fill=X)

        LeaveListButton = Button(adminmainwindow, text='Leave approval list', command=self.leavelist, bd=12,
                                 relief=GROOVE, fg="black", bg="lightblue", font=("Castellar", 36, "bold"),
                                 activebackground="lightblue")
        LeaveListButton.pack(fill=X)

        ApprovalButton = Button(adminmainwindow, text='Approve leave', command=self.LeaveApproval, bd=12, relief=GROOVE,
                                fg="black", bg="lightblue", font=("Castellar", 36, "bold"), activebackground="lightblue")
        ApprovalButton.pack(fill=X)

        LogoutBtn = Button(adminmainwindow, text='Logout', command=adminmainwindow.destroy, bd=12, relief=GROOVE,
                           fg="red", bg="lightblue", font=("Castellar", 36, "bold"), activebackground="lightblue")
    ...
    def EmployeeAllInformationWindow(self):
        ...
        cursor = conn.execute(
            'SELECT employee_id, Name, ContactNumber, employee_mail, employee_blood_group, employee_address FROM employee')
        i = 1
...
class HRClass(AdminClass):
    ...
    def HRLogin(self):
        ...
        field = []
        field = multpasswordbox(message, title, fieldnames)
        if field[0] == 'hrlogin' and field[1] == 'hrlogin':
        ...
        if choice == 'approve':
            ...
            if (col[0] == ('Emergency Leave',)):
                conn.execute("UPDATE balance SET emergencyleave =? WHERE employee_id= ?",
                             ((balance3 - exampleDays), (exampleId)))
        conn.commit()
...
class EmployeeClass:
    ...
    def EmployeeLoginWindow(self):
        ...
        informationEmployee = Button(LoginWindow, text='Employee information', command=self.EmployeeInformationWindow,
                                     bd=12, relief=GROOVE, fg="black", bg="lightblue", font=("Castellar", 36, "bold"),
                                     activebackground="lightblue")
        informationEmployee.pack(fill=X)

        submit = Button(LoginWindow, text='Submit Leave', command=self.apply, bd=12, relief=GROOVE, fg="black",
                        bg="lightblue", font=("Castellar", 36, "bold"), activebackground="lightblue")
        submit.pack(fill=X)

        LeaveBalance = Button(LoginWindow, text='Leave Balance', command=self.balance, bd=12, relief=GROOVE, fg="black",
                              bg="lightblue", font=("Castellar", 36, "bold"), activebackground="lightblue")
        LeaveBalance.pack(fill=X)

        LeaveApplicationStatus = Button(LoginWindow, text='Last leave status', command=self.EmployeeLeaveStatus, bd=12,
                                        relief=GROOVE, fg="black", bg="lightblue", font=("Castellar", 36, "bold"),
                                        activebackground="lightblue")
        LeaveApplicationStatus.pack(fill=X)

        AllLeaveStatus = Button(LoginWindow, text='All leave status', command=self.EmployeeAllStatus, bd=12,
                                relief=GROOVE, fg="black", bg="lightblue", font=("Castellar", 36, "bold"),
                                activebackground="lightblue")
        AllLeaveStatus.pack(fill=X)

        LogoutBtn = Button(LoginWindow, text='Logout', command=self.Employeelogout, bd=12, relief=GROOVE, fg="red",
                           bg="lightblue", font=("Castellar", 36, "bold"), activebackground="lightblue")
        LogoutBtn.pack(fill=X)
    ...
    def EmployeeInformationWindow(self):
        ...
        cursor = conn.execute(
            'SELECT employee_id, Name, ContactNumber, employee_mail, employee_blood_group, employee_address FROM employee where employee_id=?',
            [login])
        i = 1
        ...
        if str(date3) == fieldValues[2]:
            ...
            if choice == "Sick Leave":
                if balance1 >= (int)(fieldValues[2]):
                    conn.execute(
                        "INSERT INTO STATUS(leave_id, employee_id, leave, cur_date, Date1, Date2, days, status, hrstatus) VALUES (?,?,?,?,?,?,?,?,?)",
                        (
                        leaveid, login, choice, date_current, fieldValues[0], fieldValues[1], fieldValues[2], "Pending",
                        "Pending"))
...
class update_info_class:
    ...
    def UpdateWindow(self):
        ...
        informationEmployee = Button(UpdateWindow, text='Employee information',
                                     command=self.Employee_Information_Window, bd=12, relief=GROOVE, fg="black",
                                     bg="lightblue", font=("Castellar", 36, "bold"), activebackground="lightblue")
        informationEmployee.pack(fill=X)

        updatename = Button(UpdateWindow, text='Update Name', command=self.update_name, bd=12, relief=GROOVE,
                            fg="black", bg="lightblue", font=("Castellar", 36, "bold"), activebackground="lightblue")
        updatename.pack(fill=X)

        updatephonenumber = Button(UpdateWindow, text='Update Phone Number', command=self.update_Phone_Number, bd=12,
                                   relief=GROOVE, fg="black", bg="lightblue", font=("Castellar", 36, "bold"),
                                   activebackground="lightblue")
        updatephonenumber.pack(fill=X)

        changepassword = Button(UpdateWindow, text='Change Password', command=self.change_password, bd=12,
                                relief=GROOVE, fg="black", bg="lightblue", font=("Castellar", 36, "bold"),
                                activebackground="lightblue")
        changepassword.pack(fill=X)

        updatemail = Button(UpdateWindow, text='Update Mail ID', command=self.update_Emp_mail, bd=12, relief=GROOVE,
                            fg="black", bg="lightblue", font=("Castellar", 36, "bold"), activebackground="lightblue")
        updatemail.pack(fill=X)

        updateaddress = Button(UpdateWindow, text='Update Address', command=self.update_Emp_address, bd=12,
                               relief=GROOVE, fg="black", bg="lightblue", font=("Castellar", 36, "bold"),
                               activebackground="lightblue")
        updateaddress.pack(fill=X)

        LogoutBtn = Button(UpdateWindow, text='Logout', command=self.Employeelogout, bd=12, relief=GROOVE, fg="red",
                           bg="lightblue", font=("Castellar", 36, "bold"), activebackground="lightblue")
        LogoutBtn.pack(fill=X)
    ...
    def Employee_Information_Window(self):
        ...
        cursor = conn.execute(
            'SELECT employee_id, Name, ContactNumber, employee_mail, employee_blood_group, employee_address FROM employee where register_id=?',
            [resid])
        i = 1
...
class RegistrationClass:
    ...
    def registration(self):
        ...
        conn.execute(
            "INSERT INTO employee (register_id, employee_id, Name, ContactNumber, employee_mail, employee_blood_group, employee_address, Password) VALUES (?,?,?,?,?,?,?,?)",
            (registerid, employee_id, fieldValues[0], fieldValues[1], fieldValues[2], fieldValues[3], fieldValues[4],
             fieldValues[5]))
        conn.execute("INSERT INTO balance (employee_id, sickleave, maternityleave, emergencyleave) VALUES (?,?,?,?)",
                     (employee_id, 12, 12, 50))
...
def mainwindow():
    ...
    root.iconbitmap(default='Cred.ico')
    ...
    MainLabel = Label(root, text="Employee Leave Management System", bd=18, relief=GROOVE, fg="Black", bg="lightblue",
                      font=("Times New Roman", 58))
    MainLabel.pack(fill=X)

    AdminLgnBtn = Button(root, text='HR login', bd=12, relief=GROOVE, fg="black", bg="lightblue",
                         font=("Castellar", 36, "bold"), command=obj5.HRLogin, activebackground="lightblue")
    AdminLgnBtn.pack(fill=X)

    AdminLgnBtn = Button(root, text='Admin login', bd=12, relief=GROOVE, fg="black", bg="lightblue",
                         font=("Castellar", 36, "bold"), command=obj1.AdminLogin, activebackground="lightblue")
    AdminLgnBtn.pack(fill=X)

    # HRLgnBtn = Button(root, text='HR login',  bd=12, relief=GROOVE, fg="black", bg="yellow",font=("Castellar", 36, "bold"), command = HRLogin, activebackground = "yellow")
    # HRLgnBtn.pack(fill=X)

    EmployeeRegistration = Button(root, text='Employee registration', command=obj4.registration, bd=12, relief=GROOVE,
                                  fg="black", bg="lightblue", font=("Castellar", 36, "bold"), activebackground="lightblue")
    EmployeeRegistration.pack(fill=X)

    LoginBtn = Button(root, text='Employee login', bd=12, relief=GROOVE, fg="black", bg="lightblue",
                      font=("Castellar", 36, "bold"), command=obj2.EmployeeLogin, activebackground="lightblue")
    LoginBtn.pack(fill=X)

    updatebutton = Button(root, text="Update Information", bd=12, relief=GROOVE, fg="black", bg="lightblue",
                          font=("Castellar", 36, "bold"), command=obj3.update_information, activebackground="lightblue")
    updatebutton.pack(fill=X)

    ExitBtn = Button(root, text='Exit', command=root.destroy, bd=12, relief=GROOVE, fg="red", bg="lightblue",
                     font=("Castellar", 40, "bold"), activebackground="lightblue")
    ExitBtn.pack(fill=X)
